...
void noActionSPIInterface(uint8_t num) { __NOP(); }
void SPI_Block_rxDMA_Initialize(SPI_Block_Interface *thisInterface,
                                uint8_t thisTrig) {
  DL_DMA_setSrcAddr(thisInterface->dma, thisInterface->rxDmaChannel,
                    (uint32_t)(&thisInterface->port->RXDATA));
  DL_DMA_configMode(thisInterface->dma, thisInterface->rxDmaChannel,
                    DL_DMA_SINGLE_TRANSFER_MODE, DL_DMA_NORMAL_MODE);
  DL_DMA_setDestIncrement(thisInterface->dma, thisInterface->rxDmaChannel,
                          DL_DMA_ADDR_INCREMENT);
  DL_DMA_setDestWidth(thisInterface->dma, thisInterface->rxDmaChannel,
                      DL_DMA_WIDTH_BYTE);
  DL_DMA_setSrcIncrement(thisInterface->dma, thisInterface->rxDmaChannel,
                         DL_DMA_ADDR_UNCHANGED);
  DL_DMA_setSrcWidth(thisInterface->dma, thisInterface->rxDmaChannel,
                     DL_DMA_WIDTH_BYTE);
  DL_DMA_setTrigger(thisInterface->dma, thisInterface->rxDmaChannel, thisTrig,
                    DL_DMA_TRIGGER_TYPE_EXTERNAL);
}
void SPI_Block_txDMA_Initialize(SPI_Block_Interface *thisInterface,
                                uint8_t thisTrig) {
  DL_DMA_setDestAddr(thisInterface->dma, thisInterface->txDmaChannel,
                     (uint32_t)(&thisInterface->port->TXDATA));
  DL_DMA_configMode(thisInterface->dma, thisInterface->txDmaChannel,
                    DL_DMA_SINGLE_TRANSFER_MODE, DL_DMA_NORMAL_MODE);
  DL_DMA_setDestIncrement(thisInterface->dma, thisInterface->txDmaChannel,
                          DL_DMA_ADDR_UNCHANGED);
  DL_DMA_setDestWidth(thisInterface->dma, thisInterface->txDmaChannel,
                      DL_DMA_WIDTH_BYTE);
  DL_DMA_setSrcIncrement(thisInterface->dma, thisInterface->txDmaChannel,
                         DL_DMA_ADDR_INCREMENT);
  DL_DMA_setSrcWidth(thisInterface->dma, thisInterface->txDmaChannel,
                     DL_DMA_WIDTH_BYTE);
  DL_DMA_setTrigger(thisInterface->dma, thisInterface->txDmaChannel, thisTrig,
                    DL_DMA_TRIGGER_TYPE_EXTERNAL);
}

uint8_t SPI_Initialize_block_transfer(SPI_Block_Interface *thisInterface,
                                      SPI_Regs *thisPort,
                                      GPIO_Regs *this_gpio_port,
                                      uint8_t thisInstance, uint32_t this_csPin,
...
void SPI_Transact_block(SPI_Block_Interface *thisInterface, uint8_t *txData,
                        uint16_t numTx, uint8_t *rxData, uint16_t numRx) {
  uint8_t dummy[4];
  ...
  if (thisInterface->rxDma) {
    DL_DMA_setDestAddr(thisInterface->dma, thisInterface->rxDmaChannel,
                       (uint32_t)&thisInterface->rxBuffer[0]);

    DL_DMA_setTransferSize(thisInterface->dma, thisInterface->rxDmaChannel,
                           thisInterface->rxNum);

    DL_DMA_enableChannel(thisInterface->dma, thisInterface->rxDmaChannel);
  } else {
    DL_SPI_drainRXFIFO8(thisInterface->port, dummy, 4);
  }

  SPI_Controller_Set_CS_Low(thisInterface);

  if (thisInterface->txDma) {
    DL_DMA_setSrcAddr(thisInterface->dma, thisInterface->txDmaChannel,
                      (uint32_t)&thisInterface->txBuffer[0]);

    DL_DMA_setTransferSize(thisInterface->dma, thisInterface->txDmaChannel,
                           thisInterface->txNum);

    DL_DMA_enableChannel(thisInterface->dma, thisInterface->txDmaChannel);
  } else {
    DL_SPI_transmitData8(thisInterface->port,
                         thisInterface->txBuffer[thisInterface->tmpTxNum++]);
  }
}
  ...
  case DL_SPI_IIDX_IDLE:
    ...
    if (!thisInterface->rxDma) {
      if (thisInterface->tmpRxNum < thisInterface->rxNum) {
        thisInterface->rxBuffer[thisInterface->tmpRxNum++] =
            (uint8_t)DL_SPI_receiveData8(thisInterface->port);
      }
    ...
    if ((thisInterface->txState == SPI_IDLE) &&
        (thisInterface->rxState == SPI_IDLE)) {
      SPI_Controller_Set_CS_High(thisInterface);
